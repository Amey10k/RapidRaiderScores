...
const AddTeamForm = ({ tournamentId }) => {
  ...
  const tournamentIdFromSession = sessionStorage.getItem("TournamentId");  
  ...
  const handleTableRowClick = async (teamData) => {
    ...
    try {
        const response = await sendRegisteredTeam(tournamentIdFromSession, teamData.teamId);
  ...
  return (
    <div style={{paddingLeft:'5%', paddingRight:'10%', paddingTop:'2%', paddingBottom:'0.5%'}}>
        <Form>
