...
const StartTournament = () => {
    ...
    const handleStartButtonClick = () => {
        ...
        navigate('/match', { state: { selectedTeams }});
    };
    ...
    const handleCheckboxChange = (event, team) => {
        if (event.target.checked) {
            ...
            setSelectedTeams(selectedTeams.filter(selectedTeam => selectedTeam !== team.team.teamName));
        }
    };
    ...
    return (
        <div className="container">
            ...
            <Table striped bordered hover style={{ marginTop: '3%', borderCollapse: 'collapse', textAlign: 'center' }}>
                <thead style={{ backgroundColor: '#f8f9fa' }}>
                    <tr>
                        <th style={{ padding: '10px', borderBottom: '1px solid #dee2e6' }}>Select</th>
                        <th style={{ padding: '10px', borderBottom: '1px solid #dee2e6' }}>Team ID</th>
                        <th style={{ padding: '10px', borderBottom: '1px solid #dee2e6' }}>Team Name</th>
                    </tr>
                </thead>
                    ...
                    {teamTourJointTableData.map((team, index) => (
                        <tr key={index} style={{ borderBottom: '1px solid #dee2e6' }}>
                            <td style={{ padding: '10px' }}>
                                <input
                                    type="checkbox"
                                    checked={selectedTeams.includes(team.team.teamName)}
                                    onChange={(event) => handleCheckboxChange(event, team)}
                                />
                            </td>
                            <td style={{ padding: '10px' }}>{team.team.teamId}</td>
                            <td style={{ padding: '10px' }}>{team.team.teamName}</td>
            ...
            <div>
                <Button disabled={selectedTeams.length !== 2} onClick={handleStartButtonClick}>Start match</Button>
            </div>
        </div>
    );
};
